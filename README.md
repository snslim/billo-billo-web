...
**벡터 검색**으로 시작했습니다. Gemini Embedding API로 벡터화 후 코사인 유사도로 상위를 선택합니다.

**문제가 생겼습니다.** "접대비 매입세액 불공제 제39조"를 검색하면, 정작 제39조(불공제)보다 제38조(공제)가 상위에 올라왔습니다. 두 조항은 "매입세액"이라는 핵심 개념을 공유하기 때문에 임베딩 벡터의 코사인 유사도가 비슷했고, 벡터 검색만으로는 "공제"와 "불공제"의 의미 차이를 구분하지 못했습니다.

**원인은 명확했습니다.** 벡터 검색은 의미적 유사도에 강하지만, "제39조", "불공제"처럼 **특정 키워드의 정확한 매칭이 필요한 질의에서는 구조적으로 한계**가 있었습니다. 세무 도메인에서 조문 번호를 직접 참조하는 질의는 흔하기 때문에, 이 문제를 방치할 수 없었습니다.

**키워드 검색을 추가**하여 태그·출처·내용에 대한 정확 매칭으로 보완했습니다. 그런데 벡터 검색과 키워드 검색의 점수 스케일이 다릅니다. 코사인 유사도는 0~1 범위이고, 키워드 매칭률은 토큰 단위 비율입니다. 단순 합산은 한쪽에 편향될 수밖에 없어, **점수가 아닌 순위**를 기준으로 통합하는 **RRF(Reciprocal Rank Fusion)** 를 적용했습니다. 이후 "제39조 불공제" 질의에서 제39조가 정확히 1순위로 올라오는 것을 확인했습니다.

임베딩 API 장애 시 키워드 검색만으로 fallback하며, 법률 조항 임베딩은 변하지 않으므로 **캐싱**합니다.
