...
이미지(JPG, PNG) 또는 PDF를 업로드합니다. 세금계산서 없이도 **데모 모드**(4개 시나리오)로 전체 플로우를 체험할 수 있습니다.
...
OCR로 6개 필드를 자동 추출하고, 필드별 신뢰도(high/medium/low/missing)를 표시합니다. OCR 실패 시 직접 입력이 가능합니다.
...
**해결** — 핵심 9개 조항으로 시작했으나, 테스트에서 시행령 세부 규정이 필요한 질의를 커버하지 못했습니다. 시행령(제68조 전송기한, 제75조 등록 전 매입 예외), 가산세, 영세율·겸업·대손까지 **25개 조항으로 확장**하고, 각 조항에 키워드 태그·역할별 가중치·카테고리를 부여했습니다.
