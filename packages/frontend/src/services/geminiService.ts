...
const getRAGSystemPrompt = (
  ...
  if (role === 'receiver') {
    criticalInstructions += "- [필수 명시] summary의 첫 문장으로 다음을 명시하세요: '본 조언은 세금계산서 수취분에 한하며, 신용카드매출전표 발행공제나 의제매입세액 공제 등은 별도 검토 대상이므로 제외됩니다.'\n";
    criticalInstructions += "- [필수 경고] warnings 배열에 다음 내용을 반드시 포함하세요: '세금계산서 수취만으로는 공제받을 수 없습니다. 부가가치세 신고 시 반드시 <매입처별 세금계산서 합계표>를 제출해야 하며, 미제출/부실기재 시 매입세액 불공제 및 가산세가 부과됩니다.'\n";

    if (issues.includes('오류') || issues.includes('불일치') || issues.includes('실패') || issues.includes('미등록')) {
      criticalInstructions += "- [치명적] 시스템 검증 결과(issues), 필요적 기재사항(공급자 등록번호/성명, 공급받는자 등록번호, 작성연월일, 공급가액, 세액) 중 오류가 발견되었습니다. deductionStatus를 'impossible'로 설정하고, 사유를 명확히 설명하세요.\n";
    }

    if (userAnswers.purposeForBusiness === false) {
      criticalInstructions += "- 사용자가 '과세 사업을 위한 지출' 항목에 체크하지 않았습니다. 사업과 직접 관련이 없거나 면세 사업 관련 지출이므로 매입세액 공제 'impossible'로 판단하세요.\n";
    }

    if (userAnswers.preRegistration) {
      criticalInstructions += "- [중요] 사용자가 '사업자 등록 전 매입'을 체크했습니다. 원칙적으로는 불공제(impossible)이나, '공급시기가 속하는 과세기간 종료 후 20일 이내에 등록 신청한 경우 역산하여 공제 가능'하다는 예외 규정(부가가치세법 시행령 제75조)을 checklists와 조언에 포함하여 사용자가 놓치지 않도록 하세요.\n";
    }

    if (userAnswers.specificNonDeductible) {
      criticalInstructions += "- 사용자가 '접대비/토지/소형차 관련'임을 체크했습니다. 매입세액 불공제 대상이므로 deductionStatus를 'impossible'로 설정하고 법적 근거를 제시하세요.\n";
    }

    criticalInstructions += "- warnings 배열에 '지연 수취 가산세(0.5%)'와 '합계표 불성실 가산세'의 위험성을 반드시 경고하세요.\n";

  } else if (role === 'supplier') {
    if (userAnswers.transmittedOnTime === false) {
      criticalInstructions += "- 사용자가 '국세청 전송 기한 경과'를 시인했습니다. 지연전송 가산세 또는 미전송 가산세에 대해 강력히 경고하세요.\n";
    }
  }
...
export const getTaxAdvice = async (
  ...
  if (role === 'receiver') {
    if (userAnswers.purposeForBusiness === false) queryParts.push("사업 무관 지출 면세사업 관련 매입세액 불공제");
    if (userAnswers.preRegistration) queryParts.push("사업자 등록 전 매입세액 공제 예외 20일 이내 역산");
    if (userAnswers.specificNonDeductible) queryParts.push("접대비 비영업용 소형승용차 토지 관련 매입세액 불공제");
    queryParts.push("매입처별 세금계산서 합계표 미제출 가산세 부가가치세법 제54조");
    queryParts.push("필요적 기재사항 누락 불공제");
    queryParts.push("매입자 지연수취 가산세 제60조");
  }

  if (role === 'supplier' && userAnswers.transmittedOnTime === false) {
  ...
  try {
    ...
    try {
      ...
      advisory = {
        summary: typeof parsed.summary === 'string' ? parsed.summary : MOCK_ADVISORY_RESPONSE.summary,
        checklists: Array.isArray(parsed.checklists) ? parsed.checklists.filter((c: unknown) => typeof c === 'string') : MOCK_ADVISORY_RESPONSE.checklists,
        warnings: Array.isArray(parsed.warnings) ? parsed.warnings.filter((w: unknown) => typeof w === 'string') : MOCK_ADVISORY_RESPONSE.warnings,
        deductionStatus: ['possible', 'impossible', 'check_required'].includes(parsed.deductionStatus)
